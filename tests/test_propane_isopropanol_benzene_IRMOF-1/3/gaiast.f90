...
! }}
 ...
 nisothermpure1: do ii=1,ncomponents
  write (test_name, '( "isoterma", I1, ".dat" )' ) ii
  write(6,'(a15,1x,a50)') test_name,ajuste(ii)
  call system ("cp " // test_name // " isotermaN.dat")
...
! ...
 ...
 real function model(a,n,xx,funk)
  ...
  select case (funk)
   ...
   case("langmuir_freundlich") ! Also known as the Sips equation
   ...
   case("toth")
    ! REF.  Toth, State equations of the solid gas interface layer, Acta Chem. Acad. Hung.
    !       69 (1971) 311–317.
...
module mod_genetic
 ...
 real,parameter           :: ga_eliterate= 0.25, GA_DisasterRate = 0.0 !0.0000001
 ...
 contains
  type(typ_ga) function new_citizen(compound,seed,funk)
   implicit none
   integer           :: i,j,compound,seed
   character(len=80) :: funk
   new_citizen%genotype  = ' '
   new_citizen%phenotype = 0.0
   make_life: do while (.true.) 
    select case (funk)
     case("langmuir_freundlich")
      ...
      if( new_citizen%phenotype(1) < 0.or.new_citizen%phenotype(2)< 0 &
        .or.new_citizen%phenotype(3)< 0.or.new_citizen%phenotype(3)>1 ) cycle make_life
     ...
     case("langmuir_freundlich_dualsite")
      ...
      if(   new_citizen%phenotype(1)< 0.or.new_citizen%phenotype(2)< 0 &
        .or.new_citizen%phenotype(3)< 0.or.new_citizen%phenotype(4)< 0 & 
        .or.new_citizen%phenotype(5)< 0.or.new_citizen%phenotype(6)< 0 &
        .or.new_citizen%phenotype(3)> 1.or.new_citizen%phenotype(6)> 1 &
        ) cycle make_life
      new_citizen%fitness = fitness( new_citizen%phenotype,compound)
      exit make_life
     case("toth")
      ...
      if( new_citizen%phenotype(1) < 0 .or. new_citizen%phenotype(2)< 0 &
        .or.new_citizen%phenotype(3)< 0 ) cycle make_life
      new_citizen%fitness = fitness( new_citizen%phenotype,compound)
      exit make_life
     case("langmuir")
     do i = 1, 32*np(compound)
     ...
     if( new_citizen%phenotype(1)<0.or.new_citizen%phenotype(2)< 0 &
       .or.new_citizen%phenotype(1)>=1.0E30                        &
       .or.new_citizen%phenotype(2)>=1.0E30) cycle make_life
     new_citizen%fitness = fitness( new_citizen%phenotype,compound)
     exit make_life
     case default
      do i = 1, 32*np(compound)
...
10  arr(i+1)=a
  ...
  subroutine Mate(compound,step)
   integer             :: i, i1, i2, spos, step
   integer, intent(in) :: compound
   real                :: rrr
   logical             :: flag_disaster =.false.
   call Elitism()
   life: do i = GA_ELITISTS + 1, ga_size
    ! Crossover:
    ...
    else if ( rrr >= GA_MutationRate .and. rrr <= GA_MutationRate + GA_DisasterRate .and.&
     step >= 11 ) then
     flag_disaster = .true.
     exit life
    end if
  ...
  subroutine Fit(Compound,Seed,funk)
   implicit none
   ...
   kk = 0
   ii = 0
   pop_alpha = [(new_citizen(compound,seed,funk), i = 1,ga_size)]
   parents =>  pop_alpha
   children => pop_beta
...
" "
 ...
 if(flag)then
  ...
  do ii = 1, ncomponents
   write(6,'("Fitting compound:",1x,i2,1x,"Model:",1x,a50)') ii,ajuste(ii)
   write(6,'(a14,1x,a24,1x,a24,10x,a14)')'Compound/Step:','Cromosome:','Parameters','Control'
   call fit(ii,seed,ajuste(ii))
  end do
 end if
 call bounds()
