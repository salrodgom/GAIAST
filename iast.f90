 ...
 SELECT CASE (ajuste)
        ...
        CASE ("toth") !n = nmax*alfa*P/(1+(alfa*P)^c)^(1/c)_________________________________
          ...
          !-> isoterma2
              ...
              call system ("grep 'alfa            =' fit.log  | tail -n1 | awk '{print $3,$5}' >> coeficientes")
              call system ("grep 'c               =' fit.log  | tail -n1 | awk '{print $3,$5}' >> coeficientes")
              !CALL SYSTEM ("tail -11 fit.log | head -1 | awk '{print $3}' > coeficientes")
              !CALL SYSTEM ("tail -10 fit.log | head -1 | awk '{print $3}' >> coeficientes")
              !CALL SYSTEM ("tail -9 fit.log | head -1 | awk '{print $3}' >> coeficientes")
...
!              WRITE(6,*)(coef1(i),i=0,2)
!              WRITE(6,*)(coef2(i),i=0,2)
        ...
        CASE ("jensen") !n = K*P/(1+(K*P/(alfa*(1+k*P)))**c)**(1/c)__________________________
          ...
          !-> isoterma1
              ...
              CALL SYSTEM ("grep 'c               =' fit.log  | tail -n1 | awk '{print $3,$5}' >>  coeficientes")
!c               = 0.498684         +/- 0.001781     (0.357%)
!K1              = 11.3427          +/- 6.265        (55.23%)
!alfa            = 0.719587         +/- 14.23        (1977%)
!k2              = 81.3414          +/- 1642         (2018%)
              OPEN(unit=100,file="coeficientes",status='old')
                   READ(100,*)coef1(0)  !K1
...
! WRITE(6,*) '============================================================'
 ...
 OPEN(unit=104,file='adsorcion')

 DO i=1,intervalos
      DO j=1,intervalos

          comp = abs(PI1(i)-PI2(j))
          !comp = abs(1-(real(PI1(i))/real(PI2(j))))
...
!               WRITE(6,*) ''
!               WRITE(6,*) 'P1 =',presion1,'   pi1 =',PI1(i)
!               WRITE(6,*) 'P2 =',presion2,'   pi2 =',PI2(j)
!               WRITE(6,*) ''
!               WRITE(6,*) '-------------------------------------------------------------'
!               WRITE(6,*) ''
...
!               WRITE(6,*) 'P =',p
!               WRITE(6,*) ''
!               WRITE(6,*) '      x1 =',concx1,'     ','x1+x2 =',concx1+concx2
!               WRITE(6,*) '      x2 =',concx2
!               WRITE(6,*) '' 
!               WRITE(6,*) '      n1 =',n1,'     ','    n =',n
!               WRITE(6,*) '      n2 =',n2
